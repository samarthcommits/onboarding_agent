...
(function() {
    ...
    let get_total_data=localStorage.getItem("data");
    let data_array = JSON.parse(get_total_data);

    let get_received_data=localStorage.getItem("received");
    ...
    {
        localStorage.setItem("received",string);
    }
    if(data_array != null && received_array!=null){
        if(received_array.length<data_array.length){
            var temp=[];
            temp.push("I have successfully opened the link you requested. Let me know if you have any further questions");
            //received_array.pop();
    ...
    var toggle=localStorage.getItem("open");
    if(toggle === null)
    {
    ...
    chat.addEventListener('click', function handleClick(event) {
        ...
        if(element.className === 'chatbot_none')
        {
            ...
            var api='http://localhost:8065/suggest';
            ...
            fetch(api, {
                ...
                body: JSON.stringify({"text": window.location.pathname})
            })
                .then(res => res.json())
                .then(json =>{
                loader.remove();
                ...
                if(options.length !== 0)
                {
                    ...
                    for(var i=0;i<options.length;i++)
                    {
                        ...
                        ele.addEventListener('click', function handleClick(event){
                            ...
                            localStorage.setItem("data", string_array);
    ...
    {
        var api='http://localhost:8075/get_response';
